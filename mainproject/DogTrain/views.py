...
def up(request):
    ...
    destination = open("D:/dogtrain/mainproject/static/predic_Img/" + ran_name, 'wb+')  # 打开特定的文件进行二进制的写操作
    # 分块写入文件
...
def pred(request):
    if request.method == "POST":
        img_name = up(request)
        # 调用神经网络
        Pred = Prediction(PredictFile=r"D:/dogtrain/mainproject/static/predic_Img/"+img_name,
                          ModelFile="D:/dogtrain/mainproject/ResNet/resnet50_best.h5")
        # 预测结果
